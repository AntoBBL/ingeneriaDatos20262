...
DROP DATABASE IF EXISTS ventas_join;
...
INSERT INTO clientes (nombre, apellido, email, ciudad, fecha_registro) VALUES
  ...
  ('Sofía', 'Ramírez', 'sofia@correo.com', 'Bogotá',       '2026-02-10'),    ('Diego', 'Castro',  'diego@correo.com', 'Barranquilla', '2026-05-05');  INSERT INTO vendedores (nombre, zona) VALUES
  ('Carlos Ruiz', 'Norte'),
  ('Diana León',  'Sur'),
  ('Laura Mesa',  'Centro');   
...
1. Punto 2: ¿qué pasaría con un INNER JOIN?
La consulta devolvería 0 filas un INNER JOIN excluye de entrada a los estudiantes sin inscripción, 
entonces nunca habria un NULL para filtrar con WHERE ins.id IS NULL.

2. Punto 4: ¿por qué "React avanzado" tiene una nota promedio NULL?
No es un error: las dos inscripciones a ese curso tienen nota_final = NULL (curso aún no terminado), 
y AVG() ignora los NULL — si solo hay NULL, no hay nada que promediar, entonces el resultado es NULL.

3. Punto 6: ¿AVG(nota_final) tiene en cuenta las inscripciones sin nota?
No, AVG ignora automáticamente los NULL. Para comprobarlo, comparar COUNT(*) (todas las filas) y COUNT(nota_final) (solo las filas que no son NULL)  
si difieren, eso confirma que hay NULL excluidos del calculo.